...
XLSX_HEADERS = ["sr_no.", "Elevation", "Cam angle", "Table angle", "Zoom", "Focus", "Flash", "Part"]
...
def load_parts():
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        part_name = row[7] if len(row) > 7 else None
        if not part_name or str(part_name).strip() == "" or str(part_name).strip().lower() == "part":
            continue
        name = str(part_name).strip()
        parts[name] = {
            "elevation":   _to_num(row[1]),
            "cam_angle":   _to_num(row[2]),
            "table_angle": _to_num(row[3]),
            "zoom":        _to_num(row[4]),
            "focus":       _to_num(row[5]),
            "flash":       _to_num(row[6]),
        }
...
def save_part(name, data):
    ...
    # Find existing row by Part name (col 8 = index 7)
    target_row = None
    for row in ws.iter_rows(min_row=2):
        if len(row) > 7 and str(row[7].value).strip() == name:
            target_row = row[7].row
            break
